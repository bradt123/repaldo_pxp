...
Phx.vista.DocumentoWf=Ext.extend(Phx.gridInterfaz,{
            ...
            config:{
                ...
                renderer:function (value, p, record, rowIndex, colIndex){
                    console.log('value',record);
                        if(record.data.firma_digital != '' && record.data.solo_lectura == 'firm'){
                        ...
                        }else if(record.data.firma_digital == '' && record.data.solo_lectura == 'firm'){
                            return "<div style='text-align:center'><img border='0' style='-webkit-user-select:auto;cursor:pointer;' title='Visualizar Documento' src = '../../../lib/imagenes/icono_awesome/view_pdf.png' align='center' width='30' height='30'></div>";
                        }
                },            
            },
            type:'TextField',
            filters:{pfiltro:'dwf.firma_digital',type:'string'},
            id_grupo:1,
            grid:true,
            form:false
        },        
	...
	VisorArchivo : function(rec) {
        ...
        var name = rec.data.nombre_tipo_documento;
        ...
        if(action != '' && url =='' && ext == 0){            
            ...
            this.servicioValidadorFirmaDigital(base, url_send_view, name);
            });
        }        
    },
    ...
    checkToFirm: function (rec) {      
        var that = this;
        var action = rec.action;
        var chequeado = rec.chequeado;
        var url = rec.url;        
        ...
        if (chequeado =='si' && url !=''){
            ...
            data += "&url="+rec.url;
            url_send_view = `../../../lib/lib_control/CTOpenFile.php?${data}`;           
            var consul_fetch = fetch(url_send_view).then(response => response.arrayBuffer()).then((data) => {
                var base64 = window.arrayBufferToBase64(data)
                this.FirmaDigital(base64, this.objList);
          });            
        }else if( chequeado == 'no' && action != ''){
            Ext.Ajax.request({
            url:'../../'+rec.action,
            params:{'id_proceso_wf':rec.id_proceso_wf, 'action':rec.action},
    ...
    saveDocumentpdf: function (pdfbase, name_doc, firma){        
		...
		Ext.Ajax.request({
			url : '../../sis_workflow/control/DocumentoWf/insertDocumentFirmaDigiWf',
            success : this.fileSavepdf,
			failure : this.conexionFailure,
            params:{pdf:pdfbase,
                    name_pdf: objres.nombre,
                    id_documento_wf: objres.id_documento,
                    firma_digital: firma
                    },
			timeout : this.timeout,
			scope : this
    ...
    fileSavepdf:function(data){
        var reg = Ext.util.JSON.decode(Ext.util.Format.trim(data.responseText));
        if(!reg.ROOT.error){
             this.reload();
        }
        console.log('data_devuelta ',data);
        console.log('thisss',this);        
    },       
    FirmaDigital: function(pdf, objList) {         
        var lista = JSON.stringify(objList);
        if(pdf){
            var that = this;      
            return axios({
            ...
            .then(function(response) {                
                ...
                if (file.base64 != '' || file.base64 != null ) {                        
                        that.saveDocumentpdf(file.base64, file.name, 'boa_firma_digital');                   
                } else {
